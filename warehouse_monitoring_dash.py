...
def generate_data(n=1000):
    """
    Generates a Pandas DataFrame with simulated warehouse data for the last 30 days.
    More recent data is generated more frequently (70% from last 7 days).
    """
    now = datetime.now()
    
    timestamps = []
    for _ in range(n):
        # 70% chance for data to be from last 7 days, 30% from earlier
        if random.random() < 0.7:
            random_days = random.randint(0, 7)
        else:
            random_days = random.randint(8, 30)
        random_hours = random.randint(0, 23)
        random_minutes = random.randint(0, 59)
        timestamps.append(now - timedelta(days=random_days, hours=random_hours, minutes=random_minutes))
...
def update_dashboard(start_date, end_date, machine_id, error_codes):
    ...
    # Status Over Time Chart (Last 30 Days)
    now = datetime.now()
    thirty_days_ago = now - timedelta(days=30)
    
    # Filter to last 30 days and create date column
    status_time_df = filtered_df[filtered_df['timestamp'] >= thirty_days_ago].copy()
    status_time_df['date'] = status_time_df['timestamp'].dt.date
    
    # Calculate daily percentages
    daily_status = (status_time_df.groupby(['date', 'status'])['status']
                   .count()
                   .unstack(fill_value=0)
                   .assign(
                       success_pct=lambda x: x['success'] / (x['success'] + x['failure']) * 100,
                       failure_pct=lambda x: x['failure'] / (x['success'] + x['failure']) * 100
                   )
                   .stack()
                   .reset_index(name='percentage')
                   .rename(columns={'level_2': 'status_type'}))
    ...
    status_over_time_fig = px.line(
        ...
        y='percentage', 
        color='status',
        title='Daily Task Status Trends (Last 30 Days)',
    ...
    # Format axes
    status_over_time_fig.update_xaxes(
        tickformat="%b %d",
        dtick="D3",  # Show every 3 days
        tickangle=45
    )
    status_over_time_fig.update_yaxes(
        ticksuffix="%",
        range=[0, 100]
    )
    
    # Add today's date marker
    status_over_time_fig.add_vline(
        x=now.date(),
        line_dash="dot",
        line_color="gray",
